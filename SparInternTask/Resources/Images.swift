//
//  Images.swift
//  SparInternTask
//
//  Created by Александр Плешаков on 09.08.2024.
//

import SwiftUI

enum Images {
    static let gridLayout = UIImage(named: "GridLayout") ?? UIImage()
    static let listLayout = UIImage(named: "ListLayout") ?? UIImage()
    
    // Market
    static let addCart = UIImage(named: "AddCart") ?? UIImage()
    static let favorites = UIImage(named: "Favorites") ?? UIImage()
    static let minus = UIImage(named: "Minus") ?? UIImage()
    static let orderList = UIImage(named: "OrderList") ?? UIImage()
    static let perAmount = UIImage(named: "PerAmount") ?? UIImage()
    static let plus = UIImage(named: "Plus") ?? UIImage()
    static let star = UIImage(named: "Star") ?? UIImage()
}
